...
function splitIntoSections(parsedRows, { repeatFirstStopStartsNewSection = true } = {}) {
  if (!parsedRows.length) {
    return [];
  }

  if (!repeatFirstStopStartsNewSection) {
    return [parsedRows];
  }
...
export function parseSharedTimetable(
  { pageNumber, lineId, direction, dayType, pageText, pageItems },
  {
  ...
  return splitIntoSections(parsedRows, { repeatFirstStopStartsNewSection }).flatMap((sectionRows) =>
    buildTripsFromRows(sectionRows, {
      pageNumber,
      lineId,
      direction,
